...
Every number on this page comes from a held-out eval or benchmark committed in
the named repo and regenerated by the listed command from a clean checkout.
`python3 tools/collect_metrics.py` reads the committed metrics artifacts and
reprints four of these rows directly (constellation-vision, constellation-stream,
groundstation-rag, groundstation-train); the other four are each backed by a
committed artifact or test in their own repo but are not yet read by the
collector.
...
|---|---|---|---|
| [constellation-vision](https://github.com/maisymylod/constellation-vision) | defect-segmentation quality, held-out frames | **mean IoU 0.8821** (per-class 0.76 to 0.95) | `make eval` |
| [constellation-stream](https://github.com/maisymylod/constellation-stream) | stream throughput + query latency | **22,388 rec/s, p99 1.31 ms at 1,000,000 rows** | `make bench` |
...
scores 0.0. Similarly, `constellation-stream`'s headline is the 1,000,000-row
CI-scale run its automation regenerates; a larger 5,000,000-row run is committed
beside it in that repo, labelled with the machine it ran on, because throughput
is hardware-bound and nothing regenerates that larger run automatically. The path
to billions (parallelism, partitioned topics, object storage) is documented, not
claimed as executed. See each repo's "What is real vs simulated" section.
